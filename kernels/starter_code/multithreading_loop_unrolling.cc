        ...
        for (int col = mat_args->start; col < mat_args->end; col += 4) {
            ...
            for (int ch = 0; ch < k;) {
                ...
                for (int qj = 0; qj < 32; qj++) {
                    ...
                    signed char w_de_0_2nd = (packed_int4_0 >> 4) - 8.0;
                    signed char w_de_1_2nd = (packed_int4_1 >> 4) - 8.0;
                    signed char w_de_2_2nd = (packed_int4_2 >> 4) - 8.0;
                    signed char w_de_3_2nd = (packed_int4_3 >> 4) - 8.0;

                    intermediate_sum0_2nd += a_int8[qj + 32] * w_de_0_2nd;
                    intermediate_sum1_2nd += a_int8[qj + 32] * w_de_1_2nd;
                    intermediate_sum2_2nd += a_int8[qj + 32] * w_de_2_2nd;
                    intermediate_sum3_2nd += a_int8[qj + 32] * w_de_3_2nd;
                    // TODO: int8 multiply and accumulate operation
                }
                // dequantize the sum into floating point
                acc0 += (float)intermediate_sum0 * s_a * s_w0;
...
void MatmulOperator::mat_mul_multithreading_loop_unrolling(struct matmul_params *params) {
    ...
    int remaining_cols = n % num_thread;
    // Create threads
