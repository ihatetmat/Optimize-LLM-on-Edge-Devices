...
void MatmulOperator::mat_mul_simd_programming(struct matmul_params *params) {
    ...
    for (int row = 0; row < m; row++) {
        for (int col = 0; col < n; col++) {
            ...
            //            low|----------------------------------------------------------|high
            //               0                         256 bit
            __m256 acc0 = _mm256_setzero_ps(); // acc 값을 0으로 초기화
            // pointer of the int4 weights
            const __m256i *w_start = (__m256i *)&B->int4_data_ptr[col * k / 2];  //weight 연산 시작점
            // pointer of the int8 activation
            const __m256i *a_start = (__m256i *)&A->int8_data_ptr[row * k];  //  activation  연산 시작점
            // scale of weight
            float *s_ptr = &params->scales[col * k / 32];  // weight랑 activation scaling factor
            // scale of activation
            ...
            for (int q = 0; q < num_block; q += 2) {
                ...
                */
                ...
                __m256i raw_w = _mm256_loadu_si256(w_start); // (1)
