    ...
    for (int row = 0; row < m; row++) {
        for (int col = mat_args->start_j; col < mat_args->end_j; col++) {
            ...
            for (int q = 0; q < num_block; q += 4) {
                ...
                __m256i lower_half = _mm256_and_si256(raw_w, lowMask); // (2)
                __m256i upper_half = _mm256_srli_epi16(raw_w, 4);  // (3)
                upper_half = _mm256_and_si256(upper_half, lowMask);
                __m256i lower_half_2 = _mm256_and_si256(raw_w_next, lowMask); // (2)
                __m256i upper_half_2 = _mm256_srli_epi16(raw_w_next, 4);  // (3)
                upper_half_2 = _mm256_and_si256(upper_half_2, lowMask);
                ...
                w_0 = _mm256_sub_epi8(lower_half, zero_point);
                w_128 = _mm256_sub_epi8(upper_half, zero_point);
                w_0_next = _mm256_sub_epi8(lower_half_2, zero_point);
                w_128_next = _mm256_sub_epi8(upper_half_2, zero_point);
...
void MatmulOperator::mat_mul_all_techniques(struct matmul_params *params) {
    ...
    int remaining_cols = C->column % num_thread;
    // Create threads
